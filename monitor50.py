...
def kontrollerile(id,inn,addr): # uurime vastuse voimalikkust ja tekitame newstate sisu teadaoleva mac aadressiga kontrolleri jaoks
    ...
    if inn == "":
        sdata = "id:" + id + "\n" # alustame vastust id-ga, in pole vaja
    else: # inn on olemas
        print 'returning inn',str(inn) ##
        if 'str' in str(type(inn)) and len(inn) > 1:
            sdata = "id:" + id + "\nin:" + inn + "\n" # yks in:
        elif 'list' in str(type(inn)):
            sdata = "id:" + id + "\n"
            for ins in inn:
                sdata +=  "in:" + ins + "\n" # mitu in yhes sonumis kontrollerile
            print 'multi-in', sdata
        else:
            print 'unknown type for inn for id',id
    ...
    try: # kas on infot udp sisendpuhvris, ootame timeout vorra mis alguses defineeritud. kui ei ole, saatma!
        ...
        for data in indata: # splits into pieces with id and in if the latter exists
            if "id:" in data:
                ...
                print "\nfrom controller",id, t, "in", inn  # addr   # got the data, start printing lines content
